...
| Epic 2: Prompt/model config collections | MVP | Define Firestore collections for prompts and model configs, including versioning rules and basic validation | Decouple prompts/models from code and enable iteration | Changing `promptId`/`modelId` in a step switches behavior without deploy |
