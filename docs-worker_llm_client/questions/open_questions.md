...
| 18 | High | Yes | **Механизм инъекции контекста в Prompt:** Как именно worker передает содержимое артефактов (`ohlcv_gcs_uri`, `charts_...`) в Gemini? Скачивает JSON и вставляет как текст (Token count risk!) или передает GCS URI (Multimodal/Vertex AI feature)? | Скорее всего, скачиваем JSON и вставляем в текст промпта (text injection), т.к. Multimodal обычно для картинок/аудио, а не для сырых JSON данных. | TBD | TBD |
...
| 3 | 2025-12-24 | Канонический артефакт отчёта в GCS — **один JSON файл** по `contracts/llm_report_file.schema.json` (внутри есть `summary.markdown` и `details`). |
| 4 | 2025-12-24 | GCS naming — **один детерминированный путь** от `runId+stepId`, без `attempt` и без timestamp в имени. |
